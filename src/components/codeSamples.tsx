...
const CodeHighlighter = React.memo((sample: CodeSample) => {
  ...
  return (
    <Card className="gap-0! p-0! shadow">
      <CardHeader className="flex w-full items-center gap-2 rounded-t-lg border-b border-border bg-muted px-4 py-2 font-semibold text-foreground">
    ...
    >
      ...
      <div className="relative w-full overflow-hidden pt-4">
        ...
        >
          {Object.entries(codeSamples).map(
            ([key, sample]: [string, CodeSample]) => (
              <div key={key} className="w-full shrink-0 p-1 text-sm">
